...
%>
	<%
		...
		//2. 상단에서 폼태그 검색(search 값)
	%>
			
			<div class="mr-4">
					<img src=<%=artistInfo.get("photo") %> alt="가수 이미지" width="180px">
				</div>
				<div>
					<h3><%=artistInfo.get("title") %></h3>
					<div><%=artistInfo.get("debute") %></div>
					<div><%=artistInfo.get("agency") %></div>
				</div>
				
				<div class="mr-4">
					<img src=<%=target.get("thumbnail") %> alt="가수 이미지" width="180px">
				</div>
				<div>
					<h3><%=target.get("title") %></h3>
					<div class="text-success font-weight-bold"><%=target.get("singer") %></div>
					<div class="music-info d-flex ml-2">
						<div>
							<div>앨범</div>
							<div>재생시간</div>
							<div>작곡가</div>
